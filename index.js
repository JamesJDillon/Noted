...
class Generator {
  async init() {
    ...
    console.log('[Generating new files]'.cyan);
    await this.generatePosts(posts);
    await this.generateIndex(posts);

    console.log('--------------------------------'.rainbow);
    if (this.errors.length > 0) {
      console.log(`${this.errors.length} errors have occured.`);
      this.errors.forEach(error => console.log(`\t${error}`));
    } else {
      console.log('[SUCCESS] Your blog was successfully generated.'.rainbow);
    }
  ...
  async generatePosts(posts) {
    posts.forEach(async post => {
      try {
        ...
        console.log(`./output/${post.details.name}.html created.`.green);
      } catch (e) {
  ...
  async generateIndex(posts)  {
    ...
    console.log('index.html created.'.green);
  }
